...
const fetch = require('node-fetch') // Menggunakan node-fetch bawaan project
...
const live3dGen = async (prompt, aspectRatio = '1:1') => {
  ...
  let result
  let retryCount = 0
  const maxRetries = 15 // Maksimal waktu tunggu ~60 detik untuk mencegah bot hang
  ...
  do {
    ...
    if (retryCount >= maxRetries) {
        throw new Error('Timeout: Gagal memproses gambar dari server Live3D.')
    }
