...
const handler = async (m, { conn, usedPrefix, command, groupMetadata, isOwner, isAdmin, args }) => {
  ...
  if (command === 'done' && !m.isGroup) {
    try {
      ...
      // github.com/XM4ZE/XMYULA-MD
      const argsArray = m.text.trim().split(' ');
      if (argsArray.length > 1 && !isNaN(argsArray[1])) {
        const orderIndex = parseInt(argsArray[1]) - 1;
        ...
        await conn.sendMessage(chatId, {
          ...
          contextInfo: { mentionedJid: [userId, m.sender] }
        });
        ...
        return await conn.sendMessage(m.chat, {
          text: `Order untuk ${tagUser} di group ${groupName} telah diselesaikan!`,
          mentions: [userId],
      ...
      processingOrders.forEach((order, index) => {
        let tempTag = `@${order.userId.replace(/@.+/, '')}`;
        listText += `${index + 1}. Order dari ${tempTag} di group ${order.groupName}\n`;
      });
  ...
  /* PERBAIKAN: FORMAT TAG BERSIH UNTUK PROSES & DONE VIA QUOTED */
  let tagUser = `@${userId.replace(/@.+/, '')}`;
  let tagSender = `@${m.sender.replace(/@.+/, '')}`;
  ...
  try {
    ...
    if (/^proses$/i.test(command)) {
      if (conn.orders[m.chat][userId]) {
        return await conn.sendMessage(m.chat, {
          text: `Order dari ${tagUser} sudah sedang diproses!`,
          mentions: [userId],
          contextInfo: { mentionedJid: [userId] }
        });
      }
    ...
    if (/^done$/i.test(command)) {
      ...
      if (!order) {
        return await conn.sendMessage(m.chat, {
          text: `Tidak ada order dari ${tagUser} yang sedang diproses!`,
          mentions: [userId],
          contextInfo: { mentionedJid: [userId] }
        });
      }
