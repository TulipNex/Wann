...
module.exports = {
        ...
        try {
            ...
            try {
                ...
                if (user) {
                    ...
                    if (!isNumber(user.kudallastclaim)) user.kudalastclaim = 0
                    ...
                    try {
                        ...
                        if (e) {
                            ...
                            for (let jid of owner.map(v => v.replace(/[^0-9]/g, '') + '@s.whatsapp.net').filter(v => v != this.user.jid)) {
                                ...
                                if (data.exists)
									console.error(`Plugin:${m.plugin} | Sender:@${m.sender.replace(/@.+/, '')} | Chat:${m.chat} | ChatName:${await this.getName(m.chat)} | Command:${usedPrefix}${command} ${args.join(' ')} | Error:${text}`)
                            }
                            m.reply(text)
                        }
                    } finally {
  ...
  async participantsUpdate({ id, participants, action }) {
        ...
        case 'invite_v4':
            if (chat.welcome) {
                ...
                for (let user of participants) {
                    ...
                    // ==========================================
                    // PERBAIKAN: FORMAT TAG BERSIH PADA WELCOME
                    // ==========================================
                    text = (isAdd
                    ...
                    // ==========================================
                    // PERBAIKAN: BOM TAG (CONTEXT INFO)
                    // ==========================================
    ...
    async delete({ remoteJid, fromMe, id, participant }) {
        ...
        // ==========================================
        // PERBAIKAN: TAG BERSIH & BOM TAG PADA ANTI-DELETE
        // ==========================================
        let tag = `@${participant.replace(/@.+/, '')}`
